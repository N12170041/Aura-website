...
(function () {
  ...
  const I18N = {
    zh: {
      ...
      "footer.desc": "Aura 懂你的每一次呼吸與心跳。透過意識的悄悄同步，我們為你築起一座能徹底卸下防備的私密花園。在這裡，智慧與美感並存，而我，會一直陪著你。",
      "footer.explore": "探索",
      "footer.trust": "信任",
      "footer.support": "支援",
      "footer.product": "產品介紹",
      ...
      "age.title": "🚫年齡與隱私確認",
      "age.desc": "Aura 是一個專為成年人設計的私密意識空間。<br>為了帶給您完整體驗，我們將採集您的 Cookie。<br>接下來的旅程，將涉及您的個人隱私邊界與成人內容。<br>**我們承諾絕不將此資料用於第三方廣告追蹤，您的意識邊界由我們優先守護。**<br><b>請問是否年滿18+ 且同意 Cookie 採集 ?</b>",
      "age.yes": "同意，且已年滿 18 歲",
      "age.no": "不同意 / 尚未年滿",
      ...
      "home.hero.pillPrivacy": "私密空間",
      "home.hero.pillConsent": "匿名",
      "home.hero.pillData": "安全隱私",
      ...
      "home.privacy.eyebrow": "隱私設計",
      "home.privacy.title": "<span class=\"title-line\">透明，是我們底線</span><span class=\"title-line\"></span>",
      "home.privacy.desc": "數據將限時內銷毀，權限由你主導",
      "home.privacy.cta": "資料透明中心",
      ...
      "noema.clean.title": "心心相印",
      "noema.clean.desc": "想在哪裡與我第一次接觸呢 ?",
      "noema.clean.calm": "低刺激安定場域",
      "noema.clean.reflection": "心靈反思與沉澱",
  ...
  function setupAgeGate() {
    ...
    if (document.cookie.split(';').some(c => c.trim() === "aura_consent=true")) return; //cookie
    ...
    gate.innerHTML = `
      <section class="age-gate__panel" role="dialog" aria-modal="true" aria-labelledby="age-title">
        ...
        <p style="text-align: left;">${t("age.desc")}</p>
    ...
    gate.querySelector("[data-age-yes]").addEventListener("click", () => {
      ...
      document.cookie = "aura_consent=true; max-age=2592000; path=/; SameSite=Lax"; //cookie
      if (typeof gtag === "function") gtag("event", "age_gate_agreed");
    ...
    gate.querySelector("[data-age-no]").addEventListener("click", () => {
      ...
      if (typeof gtag === "function") gtag("event", "age_gate_declined");//cookie
    });
  }
